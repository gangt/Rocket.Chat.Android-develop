...
public class WorkCommunicationGroupActivity extends BusinessBaseActivity implements View.OnClickListener {

    private static final int SELECT_USER = 1;
    private static final String TAG = "WorkCommunicationAct";
    private TextView tv_title, tv_create, tv_channel_remind, tv_subordinate_system, tv_add_member;
    private ImageView iv_back, iv_add_person, iv_subordinate_system;
    ...
    private void initView() {
        ...
        et_channel_name = findViewById(R.id.et_channel_name);
    ...
    private void initData() {
        tv_title.setText(R.string.work_community_group);
...
//        spannableString.setSpan(new ForegroundColorSpan(Color.parseColor("#FF0000")), 0, 1, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
     ...
     * @param companyId
     */
    private void loadData(String companyId) {//工作巢 w
        RealmLabelsRepository labelsRepository = new RealmLabelsRepository(hostname);
        labelsList = labelsRepository.getByType(RocketChatConstants.W, companyId);
        if (labelsList == null) {
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.ll_add_member:
                ...
                //工作巢 w
                intent.putExtra("type", RocketChatConstants.W);
    ...
    private void createGroup() {
        if (TextUtils.isEmpty(et_channel_name.getText().toString().trim())) {
            ToastUtils.showToast(getString(R.string.input_channel_name));
            return;
        }
        if (TextUtils.isEmpty(tv_add_member.getText())) {
        ...
        String name = et_channel_name.getText().toString();
//        s_superior_channel.isChecked();

        methodCallHelper.createWorkingGroupForMobile(userId, name, jsonArray, "", tixiString, s_isSpeak.isChecked())
                .onSuccess(task -> task.getResult(), Task.UI_THREAD_EXECUTOR)
...
//                    RCLog.d("createGroup: result=" + result, true);
                    if (!TextUtils.isEmpty(result)) {
                        // 创建成功
                        ToastUtils.showToast(getString(R.string.create_success));
                        RelationUserDataSubscriber relationUserDataSubscriber = new RelationUserDataSubscriber(WorkCommunicationGroupActivity.this, hostname, RealmStore.getOrCreate(hostname));
                        ...
                        methodCall.getRoomSubscriptions().onSuccess(task1 -> {
                            Registrable listener = new StreamNotifyUserSubscriptionsChanged(
                                    WorkCommunicationGroupActivity.this, hostname, realmHelper, RocketChatCache.INSTANCE.getUserId());
                            listener.register();
                            return null;
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == SELECT_USER && resultCode == Activity.RESULT_OK && data != null) {//正常搜索人员列表
            ...
            tv_add_member.setText(getSelectListToName());
        }
    }
