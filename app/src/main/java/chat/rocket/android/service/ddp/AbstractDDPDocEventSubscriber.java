...
public abstract class AbstractDDPDocEventSubscriber implements Registrable {
  ...
  protected JSONObject customizeFieldJson(JSONObject json) throws JSONException {
    try {
      if (json.has("allRoles")&& chat.rocket.android.helper.TextUtils.isEmpty(json.getString("allRoles"))){
        json.remove("allRoles");
      }
    } catch (JSONException e) {
    }
    try {
      if (json.has("deptRole")&& chat.rocket.android.helper.TextUtils.isEmpty(json.getString("deptRole"))){
    ...
    if(!json.isNull(RealmUser.CREATEDAT)){
      try {
        long createdAt = json.getJSONObject(RealmUser.CREATEDAT).getLong(JsonConstants.DATE);
  ...
  private void onDocumentChanged(Realm realm, DDPSubscription.Changed docEvent)
    ...
    mergeJson(json, docEvent.fields);
//    RCLog.d("->>>>>>>>"+customizeFieldJson(json));
    realm.createOrUpdateObjectFromJson(getModelClass(), customizeFieldJson(json));
  }
  ...
  private void mergeJson(JSONObject target, JSONObject src) throws JSONException,NullPointerException {
    if (src==null)
      return;
