...
public class RealmPublicSettingRepository extends RealmRepository
    ...
    public PublicSetting getPublicSettingById(String _id){
        Realm realm = RealmStore.getRealm(hostname);
        if(realm==null){
            return null;
        }

        RealmPublicSetting first = realm.where(RealmPublicSetting.class)
                .equalTo(RealmPublicSetting.ID, _id)
                .findFirst();
        close(realm,Looper.myLooper());
        if(first == null) return null;
        return first.asPublicSetting();
    }
}
