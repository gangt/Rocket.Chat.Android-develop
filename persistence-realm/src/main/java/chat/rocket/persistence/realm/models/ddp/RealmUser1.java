...
public class RealmUser1 extends RealmObject {
    ...
    public static RealmQuery<RealmUser1> queryCurrentUser(Realm realm) {
        RealmAuth realmAuth = realm.where(RealmAuth.class).findFirst();
        if(realmAuth==null){
            return realm.where(RealmUser1.class)
                   .isNotEmpty(EMAILS);
        }
        Auth auth = realmAuth.asAuth();
        return realm.where(RealmUser1.class).
                equalTo(RealmUser1.ID,auth.getId())
