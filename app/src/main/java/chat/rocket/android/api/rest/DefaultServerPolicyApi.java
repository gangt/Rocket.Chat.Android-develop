...
public class DefaultServerPolicyApi implements ServerPolicyApi {
  ...
  private Flowable<Response<JSONObject>> getApiInfo(@NonNull String protocol) {
    return Flowable.create(new FlowableOnSubscribe<Response<JSONObject>>() {
      @Override
      public void subscribe(FlowableEmitter<Response<JSONObject>> responseEmitter) throws Exception {
        final Call call = client.newCall(DefaultServerPolicyApi.this.createRequest(protocol));

        call.enqueue(DefaultServerPolicyApi.this.getOkHttpCallback(responseEmitter, protocol));
