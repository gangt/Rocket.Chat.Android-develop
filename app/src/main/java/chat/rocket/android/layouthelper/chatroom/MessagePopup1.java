...
import chat.rocket.core.interactors.DeleteMessageInteractor;
...
/**其他人*/
public class MessagePopup1 {
    private static volatile MessagePopup1 singleton = null;
    private static final Action REPLY_ACTION_INFO = new Action("回复", null, true);
    private static final Action QUOTE_ACTION_INFO = new Action("引用", null, true);
    private static final Action RELAY_ACTION_INFO = new Action("转发", null, true);
//    private static final Action EDIT_ACTION_INFO = new Action("编辑", null, false);
    private static final Action COPY_ACTION_INFO = new Action("复制", null, true);
//    private static final Action DELETE_ACTION_INFO = new Action("撤回", null, false);
    ...
    private MessagePopup1(Message message) {
...
//        EditMessageInteractor editMessageInteractor = getEditMessageInteractor(hostname);
        DeleteMessageInteractor deleteMessageInteractor = getDeleteMessageInteractor(hostname);
        ...
        Disposable disposable = messageRepository.getById(singleton.message.getId())
                .flatMap(it -> {
                    ...
                    return Single.zip(
                            Single.just(message),
                            deleteMessageInteractor.isAllowed(message),
                            Pair::create
                    );
                })
...
//                                    .setTitle("Message")
    ...
    private void addDefaultActions() {
        ...
        singleton.defaultActions.add(RELAY_ACTION_INFO);
...
//        singleton.defaultActions.add(DELETE_ACTION_INFO);
    ...
    public static MessagePopup1 take(Message message) {
        if (singleton == null) {
            synchronized (MessagePopup1.class) {
                if (singleton == null) {
    ...
    public MessagePopup1 addAction(@NonNull CharSequence actionName, ActionListener actionListener) {
    ...
    public MessagePopup1 setReplyAction(ActionListener actionListener) {
        REPLY_ACTION_INFO.actionListener = actionListener;
        return singleton;
    }

//    public MessagePopup1 setEditAction(ActionListener actionListener) {
//        EDIT_ACTION_INFO.actionListener = actionListener;
//        return singleton;
//    }

    public MessagePopup1 setCopyAction(ActionListener actionListener) {
        COPY_ACTION_INFO.actionListener = actionListener;
        return singleton;
    }

//    public MessagePopup1 setDeleteAction(ActionListener actionListener) {
//        DELETE_ACTION_INFO.actionListener = actionListener;
//        return singleton;
//    }

    public MessagePopup1 setQuoteAction(ActionListener actionListener) {
        QUOTE_ACTION_INFO.actionListener = actionListener;
        return singleton;
    }

    public MessagePopup1 setRelayAction(ActionListener actionListener) {
        RELAY_ACTION_INFO.actionListener = actionListener;
        return singleton;
    }
...
//        );
//    }
private DeleteMessageInteractor getDeleteMessageInteractor(String hostname) {
    UserRepository userRepository = new RealmUserRepository(hostname);
    RoomRoleRepository roomRoleRepository = new RealmRoomRoleRepository(hostname);
    PermissionRepository permissionRepository = new RealmPermissionRepository(hostname);
    ...
    return new DeleteMessageInteractor(
            permissionInteractor,
            userRepository,
            messageRepository,
            roomRepository,
    ...
    private static class Builder {
        ...
        public MessagePopup1 build() {
            Message message = this.message;
            return new MessagePopup1(message);
        }
    }
