...
public class RealmRoomRepository extends RealmRepository implements RoomRepository {
    ...
    public List<Room> searchHistorySetting(String companyId, String str){
        Realm realm = RealmStore.getRealm(hostname);
        if (realm == null) {
            return null;
        }
        RealmResults<RealmRoom> all = realm.where(RealmRoom.class)
                .equalTo(RealmRoom.TYPE, RealmSubscription.TYPE_SETTING)
                .and()
                .equalTo(RealmRoom.OPEN, false)
                .and()
                .notEqualTo(RealmRoom.COMPANY_ID, companyId)
                .and()
    ...
    public Flowable<List<Room>> getHistorySetting(String companyId){
        return Flowable.defer(() -> Flowable.using(
                ...
                pair -> {
                    ...
                    return pair.first.where(RealmRoom.class)
                            ...
                            .equalTo(RealmRoom.OPEN, false)
                            .and()
                            .equalTo(RealmRoom.COMPANY_ID, companyId)
                            .findAll()
                            .asFlowable();
                },
    ...
    public Flowable<List<Room>> getAll(String companyId){
        return Flowable.defer(() -> Flowable.using(
                () -> new Pair<>(RealmStore.getRealm(hostname), Looper.myLooper()),
                pair -> {
                    if (pair.first == null) {
    ...
    public Flowable<List<Room>> getAllNotNull(String companyId){
        return Flowable.defer(() -> Flowable.using(
                () -> new Pair<>(RealmStore.getRealm(hostname), Looper.myLooper()),
                pair -> {
                    if (pair.first == null) {
                    ...
                    return pair.first.where(RealmRoom.class)
                            .equalTo(RealmRoom.COMPANY_ID, companyId)
                            .and()
                            .isNotEmpty(RealmRoom.DISPLAYNAME)
                            .findAll()
                            .asFlowable();
                },
                pair -> close(pair.first, pair.second))
    ...
    public Room getByRoomId(String roomId){
        ...
        RealmRoom realmRoom = realm.where(RealmRoom.class)
                .equalTo(RealmRoom.ID, roomId)
                .findFirst();
        if(realmRoom==null){
            return null;
        }
        close(realm, Looper.myLooper());
        return realmRoom.asRoom();
    }

    public String getByRoomNameId(String roomId){
        Realm realm = RealmStore.getRealm(hostname);
        if (realm == null) {
            return null;
        }
        ...
        close(realm, Looper.myLooper());
        return realmRoom.asRoom().getName();
    }
    public Flowable<Optional<Room>> getByIdAll(String roomId) {
        return Flowable.defer(() -> Flowable.using(
                () -> new Pair<>(RealmStore.getRealm(hostname), Looper.myLooper()),
                pair -> {
                    if (pair.first == null) {
