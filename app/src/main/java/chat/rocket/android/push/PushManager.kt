...
typealias TupleRoomUser = Pair<Subscription, User>
...
object PushManager {
    ...
    private fun getContentIntent(context: Context, notificationId: Int, pushMessage: PushMessage, grouped: Boolean = false): PendingIntent {
        val notificationIntent = Intent(context, ChatMainActivity::class.java)
    ...
    class ReplyReceiver : BroadcastReceiver() {
        ...
        private fun sendMessage(hostname: String, message: CharSequence, roomId: String) {
            val subscriptionRepository = RealmSubscriptionRepository(hostname)
            val userRepository = RealmUserRepository(hostname)
            val messageRepository = RealmMessageRepository(hostname)
            val messageInteractor = MessageInteractor(messageRepository, subscriptionRepository)

            val singleRoom: Single<Subscription> = subscriptionRepository.getById(roomId)
                    .filter({ it.isPresent })
                    .map({ it.get() })
                    .firstElement()
                    .toSingle()
            ...
            val roomUserTuple: Single<TupleRoomUser> = Single.zip(
                    singleRoom,
                    singleUser,
                    BiFunction { subscription, user -> TupleRoomUser(subscription, user) })
