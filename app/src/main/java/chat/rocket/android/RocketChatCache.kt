...
object RocketChatCache {
    ...
    private val KEY_SESSIONID="KEY_SESSIONID"
    private val KEY_CLICK_MESSAGE ="KEY_CLICK_MESSAGE"
    private val KEY_CLICK_HOSTORY_INFO ="KEY_CLICK_HOSTORY_INFO"
    private val FRAME_USERID ="FRAME_USERID"
    private val IS_DOWNLINE ="IS_DOWNLINE"
    private val IS_HOMEPAGE_VISIABLE ="IS_HOMEPAGE_VISIABLE"
    private val KEY_COMPANYID="KEY_COMPANYID"
    private val KEY_ALIYUN_DEVICEID="KEY_ALIYUN_DEVICEID"
    private val KEY_AVATAR="KEY_AVATAR"
    private val KEY_CAN_CONNECT_WEBSOCKET="KEY_CAN_CONNECT_WEBSOCKET"
    ...
    fun setSessionId(sessionId: String?){
        sessionId?.let {
            setString(KEY_SESSIONID,sessionId)
        }
    }
    fun getSessionId(): String?=getString(KEY_SESSIONID,null)

    fun setIsClickInfoMsg(isClick: String?) {
        setString(KEY_CLICK_MESSAGE, isClick)
    }

    fun getIsClickInfoMsg(): String? = getString(KEY_CLICK_MESSAGE, null)

    fun setIsClickHostoryInfo(isClick: String?) {
        setString(KEY_CLICK_HOSTORY_INFO, isClick)
    }

    fun getIsClickHostoryInfo(): String? = getString(KEY_CLICK_HOSTORY_INFO, null)

    fun getFrameUserId(): String? {
        return getString(FRAME_USERID, null)
    }

    fun setFrameUserId(userId: String?) {
        userId?.let {
            setString(FRAME_USERID, userId)
        }
    }

    fun getDownLine(): Boolean? {
        return getBoolean(IS_DOWNLINE, false)
    }

    fun setDownLine(isdownLine: Boolean?) {
        isdownLine?.let {
            setBoolean(IS_DOWNLINE, isdownLine)
        }
    }

    fun setCompanyId(companyId: String?){
        companyId?.let {
            setString(KEY_COMPANYID,companyId)
        }
    }

    fun getCompanyId(): String?=getString(KEY_COMPANYID,null)

    fun setConnectStus(connectStus: String?){
        connectStus?.let {
            setString(KEY_CAN_CONNECT_WEBSOCKET,connectStus)
        }
    }

    fun getConnectStus(): String?=getString(KEY_CAN_CONNECT_WEBSOCKET, ServerConnectivity.STATE_DISCONNECTED.toString() + "")

    fun setAliyunDeviceId(deviceId: String?){
        deviceId?.let {
            setString(KEY_ALIYUN_DEVICEID,deviceId)
        }
    }

    fun getAliyunDeviceId(): String?=getString(KEY_ALIYUN_DEVICEID,null)

    fun setAvatar(avatar: String?){
        avatar?.let {
            setString(KEY_AVATAR,avatar)
        }
    }

    fun getAvatar(): String?=getString(KEY_AVATAR,null)

    fun getHomePageVisiable(): Boolean? {
        return getBoolean(IS_HOMEPAGE_VISIABLE, false)
    }

    fun setHomePageVisiable(is_homepage_visiable: Boolean?) {
        is_homepage_visiable?.let {
            setBoolean(IS_HOMEPAGE_VISIABLE, is_homepage_visiable)
        }
    }
