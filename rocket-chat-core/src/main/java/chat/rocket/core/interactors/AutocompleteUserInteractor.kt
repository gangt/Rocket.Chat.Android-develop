...
class AutocompleteUserInteractor(private val subscription: Subscription,
    ...
    private val groupMentions = listOf(
            SpotlightUser.builder().setId("all").setUsername("all").setStatus("online").setDeleted(false).build(),
            SpotlightUser.builder().setId("here").setUsername("here").setStatus("online").setDeleted(false).build()
    )
    ...
    fun getSuggestionsFor(name: String): Flowable<List<SpotlightUser>> {
        return Flowable.zip<String, List<Message>, List<SpotlightUser>, Triple<String, List<Message>, List<SpotlightUser>>>(
                Flowable.just(name),
                messageRepository.getAllFrom(subscription),
